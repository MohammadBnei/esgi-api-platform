...
export function retrieveError(retrieveError) {
  return { type: 'DRINK_UPDATE_RETRIEVE_ERROR', retrieveError };
}

export function retrieveLoading(retrieveLoading) {
  return { type: 'DRINK_UPDATE_RETRIEVE_LOADING', retrieveLoading };
}

export function retrieveSuccess(retrieved) {
  return { type: 'DRINK_UPDATE_RETRIEVE_SUCCESS', retrieved };
}
...
export function updateError(updateError) {
  return { type: 'DRINK_UPDATE_UPDATE_ERROR', updateError };
}

export function updateLoading(updateLoading) {
  return { type: 'DRINK_UPDATE_UPDATE_LOADING', updateLoading };
}

export function updateSuccess(updated) {
  return { type: 'DRINK_UPDATE_UPDATE_SUCCESS', updated };
}
...
export function mercureOpen(eventSource) {
  return { type: 'DRINK_UPDATE_MERCURE_OPEN', eventSource };
}

export function mercureMessage(retrieved) {
  return dispatch => {
    if (1 === Object.keys(retrieved).length) {
      dispatch({ type: 'DRINK_UPDATE_MERCURE_DELETED', retrieved });
      return;
    }

    dispatch({ type: 'DRINK_UPDATE_MERCURE_MESSAGE', retrieved });
  };
}
